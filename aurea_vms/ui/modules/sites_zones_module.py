...
from qfluentwidgets import CheckBox, ComboBox, FluentIcon, LineEdit, PrimaryPushButton, PushButton, StrongBodyLabel, TableWidget
...
class SitesZonesModule(QWidget):
    ...
    def _on_delete_site(self, site: Site) -> None:
        if confirm(self, "Eliminar sitio", f"¿Eliminar \"{site.name}\"? También se eliminan sus zonas."):
    ...
    def _on_delete_zone(self, zone: Zone) -> None:
        if confirm(self, "Eliminar zona", f"¿Eliminar \"{zone.name}\"? Las cámaras quedan sin asignar."):
            repository.delete_zone(zone.id)
            self._reload()
