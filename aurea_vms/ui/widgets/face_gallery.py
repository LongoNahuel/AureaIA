...
Si una deteccion nueva coincide con un ID ya capturado, la miniatura
guardada se reemplaza por la nueva SOLO si el recorte es mas grande (mas
cerca de la camara, mas resolucion util) que el que ya estaba -- para
que la galeria termine mostrando la mejor toma de cada persona para
inspeccion visual tipo forense, no la primera que se vio (tipicamente
chica y lejana). El umbral de que tan distinto tiene que verse un rostro
para catalogarlo como un ID nuevo es configurable por camara desde
Analizadores > Detección Facial, junto con un contador acumulado de IDs
(con reinicio diario programable).
...
def _geometry_signature(keypoints: tuple[tuple[float, float], ...] | None) -> np.ndarray | None:
    ...
    return np.array([np.linalg.norm(points[i] - points[j]) / eye_distance for i, j in pairs], dtype=np.float32)
...
def _with_metadata_overlay(pixmap: QPixmap, when: str, confidence: float) -> QPixmap:
    ...
    painter.drawText(
        bar_rect.adjusted(6, 0, -6, 0), Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignLeft, text
    )
    painter.end()
    return stamped
...
class FaceGallery(HeaderCardWidget):
    ...
    def _find_matching_row(self, candidate: dict, threshold: float) -> int | None:
        """Fila de la galería cuyo ID ya catalogado corresponde a este
        rostro (None si no coincide con ninguno -- ID nuevo)."""
        for i in range(self.list_widget.count()):
            data = self.list_widget.item(i).data(Qt.ItemDataRole.UserRole)
            if data is not None and _combined_difference(candidate, data) < threshold:
                return i
        return None
    ...
    def _on_detection(self, event: DetectionEvent) -> None:
        ...
        for det in faces:
            ...
            match_row = self._find_matching_row(candidate, threshold)
            if match_row is not None:
                existing = self.list_widget.item(match_row).data(Qt.ItemDataRole.UserRole)
                if candidate["area"] <= existing["area"]:
                    continue  # ya tenemos una toma igual o mejor de este ID
                track_id = existing["track_id"]
                self.list_widget.takeItem(match_row)
            else:
